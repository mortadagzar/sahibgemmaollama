...
export async function POST(request) {
  ...
  const { settings } = await getActiveProviderSettings('ai_chat');
  const { reply, stageAction } = settings
    ? await requestProviderReply(settings, message)
    : { reply: shellCopy.fallbackReply, stageAction: null };

  const finalReply = reply || shellCopy.fallbackReply;
  await insertChatMessage({ sender: 'sahibuya', content: finalReply });

  return NextResponse.json({ reply: finalReply, stageAction });
}
